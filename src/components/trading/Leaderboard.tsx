    ...
    const getUserData = async () => {
      ...
      if (isAuth && typeof window !== 'undefined') {
        ...
        if (user) {
          ...
          // Build map of leader_id -> allocation_percentage for active subscriptions
  ...
  const handleMirrorTrades = async (leaderId: string, leaderUsername: string) => {
    ...
    if (trader) {
      ...
      if (existingAllocation) {
        ...
        // When editing, add back the current allocation to available (since we're replacing it)
        // This is already handled in availableAllocation state, but we don't need to restrict the max
      } else {
        setIsEditMode(false);
    ...
    try {
      ...
      if (isEditMode) {
        ...
        const { data: existingSub } = await supabase
          ...
          .eq('leader_id', traderToMirror.id)
          .single();
        
        if (existingSub) {
          result = await CopyTradingService.updateSubscription(
            existingSub.id,
            currentUserId,
            { allocation_percentage: mirrorAllocation }
          );
        }
      } else {
        ...
        result = await CopyTradingService.createSubscription(
          currentUserId,
          traderToMirror.id,
          mirrorAllocation
        );
      ...
      if (result && result.success) {
        alert(`Successfully ${isEditMode ? 'updated' : 'started'} mirroring ${traderToMirror.username}'s trades with ${mirrorAllocation}% allocation!`);
        ...
        alert(result?.error || 'Failed to mirror trades');
      }
    } catch (error) {
      console.error('Error mirroring trades:', error);
      alert('Failed to mirror trades. Please try again.');
    } finally {
      setMirroringTrader(null);
    }
  };
                  ...
                  <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
                    ...
                    {isMirroring && (
                      <div className="text-center min-w-[60px]">
                        ...
                        <div className="text-sm font-semibold text-primary">{mirroringAllocation.toFixed(1)}%</div>
                      </div>
                    )}
                    ...
                    {currentUserId !== trader.id && (
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">{isMirroring ? 'Edit' : 'Mirror'}</span>
                      </Button>
                    )}
                  </div>
                </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 text-sm text-muted-foreground">
                    ...
                    Available: {isEditMode ? (availableAllocation + (userSubscriptions.get(traderToMirror.id) || 0)).toFixed(1) : availableAllocation.toFixed(1)}%
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative pt-1 pb-1">
                    ...
                    <input
                      type="range"
                      min="1"
                      max={Math.min(100, isEditMode ? availableAllocation + (userSubscriptions.get(traderToMirror.id) || 0) : availableAllocation)}
                      step="0.5"
                      ...
                      className="w-full h-2 bg-transparent rounded-full appearance-none cursor-pointer
                        ...
                        [&::-moz-range-track]:h-2"
                      style={{
                        background: `linear-gradient(to right, hsl(var(--primary)) 0%, hsl(var(--primary)) ${(mirrorAllocation / Math.min(100, isEditMode ? availableAllocation + (userSubscriptions.get(traderToMirror.id) || 0) : availableAllocation)) * 100}%, hsl(var(--muted)) ${(mirrorAllocation / Math.min(100, isEditMode ? availableAllocation + (userSubscriptions.get(traderToMirror.id) || 0) : availableAllocation)) * 100}%, hsl(var(--muted)) 100%)`,
                        height: '8px',
                        borderRadius: '9999px'
                      }}
                    />
                  ...
                  <div className="flex items-center justify-center gap-2">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        const maxAllocation = isEditMode ? availableAllocation + (userSubscriptions.get(traderToMirror.id) || 0) : availableAllocation;
                        if (!isNaN(val)) {
                          setMirrorAllocation(Math.min(Math.max(0.1, val), maxAllocation));
                        }
                      }}
                ...
                <div className="pt-2 border-t">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {isEditMode 
                        ? ((availableAllocation + (userSubscriptions.get(traderToMirror.id) || 0)) - mirrorAllocation).toFixed(1)
                        : (availableAllocation - mirrorAllocation).toFixed(1)
                      }%
                    </span>
                  </div>
                </div>
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={mirroringTrader === traderToMirror.id || mirrorAllocation <= 0 || mirrorAllocation > (isEditMode ? availableAllocation + (userSubscriptions.get(traderToMirror.id) || 0) : availableAllocation)}
                >
